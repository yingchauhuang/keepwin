...
class ThreadManager(models.Manager):
    ...
    def create_new(self, title, author, added_at, wiki, text, tagnames=None, is_anonymous=False, is_charged = False, cost = 0 ,featurepic=None,subtitle=None,passcode=None):
        ...
        question.add_revision(
            ...
            revised_at = added_at,
            featurepic = question.featurepic,
        )
...
#            return qs.filter(posts__post_type='question', posts__deleted=False, posts__self_question_id__in=question_ids)
    ...
    def run_advanced_search(self, request_user, search_state):  # TODO: !! review, fix, and write tests for this
        ...
        #print qs.query
        #Get the first 10 data -- Modeufy by YC 2012/4/16
        #qs = qs[:10];   #Chnage to page count to 9
        return qs, meta_data
    ...
    def get_today_contributors(self):
    ...
    def get_thread_contributors(self, thread_list):
        ...
        page_questions = Post.objects.filter(post_type='question', thread__in=thread_list)\
                            .only('id', 'thread', 'score', 'is_anonymous', 'summary', 'post_type', 'deleted').order_by('-added_at','id') # pick only the used fields
        page_question_map = {}
        for pq in page_questions:
            if not page_question_map.has_key(pq.author.id):
                page_question_map[pq.author.id] = pq
        for contributor in contributors:
            if page_question_map.has_key(contributor.id):
                contributor._question_cache = page_question_map[contributor.id]                    
        return contributors
...
class Thread(models.Model):
        ...
        if user is None or user.is_anonymous():
            ...
            if user.is_administrator() or user.is_moderator():
                ...
                if (question_user == user.id):
                    ...
                    return self.posts.get_answers().filter( (models.Q(is_private = False) | models.Q(author = user)) &
                               ( models.Q(deleted = False) | models.Q(author = user) \
                                | models.Q(deleted_by = user))
                            )
                """
    ...
    def get_summary_keepwin_html(self, search_state):
        html = self.get_cached_summary_keepwin_html()
        if not html:
            html = self.update_summary_keepwin_html()
    ...
    def get_summary_newest_html(self, search_state):
        ...
        if not html:
            html = self.update_summary_newest_html()
    ...
    def get_summary_twmode_html(self, search_state):
        html = self.get_cached_summary_twmode_html()
        if not html:
            html = self.update_summary_twmode_html()
    ...
    def get_cached_summary_twmode_html(self):
        return cache.cache.get(self.SUMMARY_TWMODE_CACHE_KEY_TPL % self.id)

    def get_cached_summary_keepwin_html(self):
        return cache.cache.get(self.SUMMARY_KEEPWIN_CACHE_KEY_TPL % self.id)
    ...
    def update_summary_twmode_html(self):
        context = {
            'thread': self,
            'question': self._question_post(refresh=True),  # fetch new question post to make sure we're up-to-date
            'search_state': DummySearchState(),
        }
        html = get_template('widgets/question_summary_twmode.html').render(context)
        # INFO: Timeout is set to 30 days:
        # * timeout=0/None is not a reliable cross-backend way to set infinite timeout
        # * We probably don't need to pollute the cache with threads older than 30 days
        # * Additionally, Memcached treats timeouts > 30day as dates (https://code.djangoproject.com/browser/django/tags/releases/1.3/django/core/cache/backends/memcached.py#L36),
        #   which probably doesn't break anything but if we can stick to 30 days then let's stick to it
        cache.cache.set(self.SUMMARY_TWMODE_CACHE_KEY_TPL % self.id, html, timeout=60*60*24*30)
        return html
    
    def update_summary_keepwin_html(self):
        context = {
            'thread': self,
            'question': self._question_post(refresh=True),  # fetch new question post to make sure we're up-to-date
            'search_state': DummySearchState(),
        }
        html = get_template('widgets/question_summary_keepwin.html').render(context)
        # INFO: Timeout is set to 30 days:
        # * timeout=0/None is not a reliable cross-backend way to set infinite timeout
        # * We probably don't need to pollute the cache with threads older than 30 days
        # * Additionally, Memcached treats timeouts > 30day as dates (https://code.djangoproject.com/browser/django/tags/releases/1.3/django/core/cache/backends/memcached.py#L36),
    ...
    def update_summary_newest_html(self):
        ...
        html = get_template('widgets/question_summary_newest.html').render(context)
        # INFO: Timeout is set to 30 days:
        # * timeout=0/None is not a reliable cross-backend way to set infinite timeout
        # * We probably don't need to pollute the cache with threads older than 30 days
        # * Additionally, Memcached treats timeouts > 30day as dates (https://code.djangoproject.com/browser/django/tags/releases/1.3/django/core/cache/backends/memcached.py#L36),
    ...
    def summary_html_twmode_cached(self):
        return cache.cache.has_key(self.SUMMARY_TWMODE_CACHE_KEY_TPL % self.id)
