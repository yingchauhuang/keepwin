...
User.add_to_class('SUMMARY_CACHE_KEY_TPL', 'user-question-summary-%d')
User.add_to_class('BLOGGER_CACHE_KEY_TPL', 'user-question-blogger-%d')
...
User.add_to_class('balance', models.DecimalField(default=0,max_digits=20, decimal_places=2))
...
User.add_to_class('receive_points',models.DecimalField(default=0,max_digits=20, decimal_places=2))
...
def user_get_cached_blogger_html(self):
        return cache.cache.get(self.BLOGGER_CACHE_KEY_TPL % self.id)

def user_user_blogger_html(self):
        html = self.get_cached_blogger_html()
        if not html:
            html = self.update_blogger_html()
...
def user_update_blogger_html(self):
        if self.NewestQ_id==None:
            question=None
        else:
        ...
        cache.cache.set(self.BLOGGER_CACHE_KEY_TPL % self.id, html, timeout=60 * 60 * 24 * 30)
        return html

def user_get_blogger_html(self):
        html = self.get_cached_blogger_html()
        if not html:
            html = self.update_blogger_html()
...
def user_get_cached_summary_html(self):
        return cache.cache.get(self.SUMMARY_CACHE_KEY_TPL % self.id)

def user_user_summary_html(self):
...
def user_update_summary_html(self):
        if self.NewestQ_id==None:
            question=None
        else:
            question=self.NewestQ
...
def user_get_summary_html(self):
        html = self.get_cached_summary_html()
        if not html:
            html = self.update_summary_html()
...
def user_post_comment(
    ...
    #Add by YC
    parent_post.increase_responses_count()
...
def user_edit_question(
    ...
    question.apply_edit(
        ...
        # Disable by YC .. Disablt the midification of charged type
        #is_charged = is_charged,
        #cost = cost, 
        featurepic=featurepic,
        is_charged=is_charged,
...
def user_visit_question(self, question=None, timestamp=None):
    ...
    except QuestionView.MultipleObjectsReturned:
        question_view = QuestionView.objects.filter(who=self, question=question)[0]
...
@auto_now_timestamp
def user_add_user_transaction(  self,
                                user=None,
                                income=0,
                                outcome=0,
                                transaction_type=0,
...
User.add_to_class('get_cached_summary_html', user_get_cached_summary_html)
User.add_to_class('update_summary_html', user_update_summary_html)
User.add_to_class('user_summary_html', user_user_summary_html)
User.add_to_class('get_summary_html', user_get_summary_html)
User.add_to_class('get_cached_blogger_html', user_get_cached_blogger_html)
User.add_to_class('update_blogger_html', user_update_blogger_html)
User.add_to_class('user_blogger_html', user_user_blogger_html)
User.add_to_class('get_blogger_html', user_get_blogger_html)
...
User.add_to_class('can_post_user', user_can_post_user)
...
User.add_to_class('add_user_transaction', user_add_user_transaction)
